...
from agents.bpo_agent.agent import BPOAgent, load_config
from agents.bpo_agent.prompts import CODI_REPORT_PROMPT

st.set_page_config(
    page_title="Agente de Inteligencia Competitiva - BPO Financiero",
    page_icon="🏭",
    layout="wide"
)

st.title("🏭 Agente de Inteligencia Competitiva - Externalización Financiera (BPO)")
st.markdown("""
**Asistente Estratégico para CODI**
Monitoriza competidores de BPO financiero (externalización contable/fiscal, digitalización, SII, loan staff), descubre nuevos players y genera informes estratégicos.
""")
...
with st.sidebar:
    ...
    extra_comp = st.text_input("Añadir otro competidor (opcional):", placeholder="Ej. Vistra")

if 'agent' not in st.session_state:
    st.session_state.agent = BPOAgent(config)
...
with tab1:
    if st.button("Ejecutar Análisis Completo", use_container_width=True):
        st.info("Iniciando motor de análisis de BPO financiero...")
        status_box = st.empty()
        def ui_callback(msg):
...
with tab2:
    st.header("Nuevos Players del Mercado BPO")
    if st.button("Escanear Nuevos Competidores", use_container_width=True):
        with st.spinner("Escaneando el mercado de externalización financiera..."):
...
with tab3:
    st.header("Informe Estratégico CODI - BPO Financiero")
    if st.button("Generar Informe Ejecutivo", use_container_width=True):
        if 'last_df' in st.session_state:
            with st.spinner("Redactando informe estratégico de BPO..."):
                report = st.session_state.agent.generate_codi_report(st.session_state.last_df)
                st.markdown("### 📑 Informe Estratégico (CODI + Gap Analysis BPO)")
                st.markdown(report)
                st.download_button("Descargar Informe (MD)", report, file_name="Informe_CODI_BPO.md")
        else:
            st.warning("Ejecuta primero el análisis para generar datos.")

with tab4:
    st.header("🔔 Monitorización de Marca")
    st.caption("Rastrea nuevos servicios, digitalización, cambios en SII/Verifactu, y M&A.")
...
if 'last_df' in st.session_state:
    st.subheader("Último Análisis de Mercado BPO")
...
st.divider()
st.subheader("💬 Chat Estratégico - BPO Financiero")
...
if prompt := st.chat_input("Pregunta sobre BPO, externalización contable, SII, digitalización..."):
